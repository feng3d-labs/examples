class TerrainMergeTest extends feng3d.Script {
    ...
    init() {
        ...
        var model = terrain.addComponent(feng3d.Renderer);
...
class Container3DTest extends feng3d.Script {
    ...
    init() {
        ...
        this.colorMaterial = this.cube.getComponent(feng3d.Renderer).material = feng3d.serialization.setValue(new feng3d.Material(), { shaderName: "color" });
...
class MousePickTest extends feng3d.Script {
    ...
    init() {
        ...
        cube.getComponent(feng3d.Renderer).material = new feng3d.Material();
        scene.gameObject.addChild(cube);
        var sphere = feng3d.GameObject.createPrimitive("Sphere");
        sphere.transform.position = new feng3d.Vector3(-1.50, 0, 0);
        sphere.mouseEnabled = true;
        sphere.getComponent(feng3d.Renderer).material = new feng3d.Material();
        scene.gameObject.addChild(sphere);
        var capsule = feng3d.GameObject.createPrimitive("Capsule");
        capsule.transform.position = new feng3d.Vector3(3, 0, 0);
        capsule.mouseEnabled = true;
        capsule.getComponent(feng3d.Renderer).material = new feng3d.Material();
        scene.gameObject.addChild(capsule);
        var cylinder = feng3d.GameObject.createPrimitive("Cylinder");
        cylinder.transform.position = new feng3d.Vector3(-3, 0, 0);
        cylinder.mouseEnabled = true;
        cylinder.getComponent(feng3d.Renderer).material = new feng3d.Material();
        scene.gameObject.addChild(cylinder);
        scene.on("click", (event) => {
            var gameObject = event.target;
            if (gameObject.getComponent(feng3d.Renderer)) {
                var uniforms = gameObject.getComponent(feng3d.Renderer).material.uniforms;
...
class PointLightTest extends feng3d.Script {
    ...
    init() {
        ...
        function initLights() {
            ...
            var model = light0.addComponent(feng3d.Renderer);
            ...
            model = light1.addComponent(feng3d.Renderer);
            model.geometry = feng3d.serialization.setValue(new feng3d.SphereGeometry(), { radius: 0.05 });
            //初始化点光源
...
class ColorMaterialTest extends feng3d.Script {
    ...
    init() {
        ...
        scene.gameObject.addChild(cube);
        //初始化颜色材质
        var colorMaterial = cube.getComponent(feng3d.Renderer).material = feng3d.serialization.setValue(new feng3d.Material(), { shaderName: "color" });
        //变化旋转与颜色
        setInterval(function () {
            cube.transform.ry += 1;
        }, 15);
...
class SegmentMaterialTest extends feng3d.Script {
    ...
    init() {
        ...
        //初始化材质
        var model = segment.addComponent(feng3d.Renderer);
