/// <reference path="libs/monaco.d.ts" />
...
(function () {
	...
	window.onload = function () {
		require(['vs/editor/editor.main'], function () {
			xhr('feng3d/release/feng3d.d.ts').then(function (response) {
	...
	function load() {
		...
		function loadSample(sampleId, callback) {
			...
			samplePath = 'js/examples/' + samplePath;
	...
	function doRun(runContainer) {
		...
		runIframe.addEventListener('load', function (e) {
			runIframe.contentWindow.load(getLang('js'));
		});
	}
